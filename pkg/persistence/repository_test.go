...
func TestRepository_PromoteOfferingsQA_MyHome텍스트임대조건을승인한다(t *testing.T) {
	repo := openCleanTestRepository(t)
	ctx := context.Background()
	noticeID := insertSourceNoticeFixture(t, repo, "19833", "2026년 신혼·신생아 전세임대 I 입주자 수시모집 공고")
	artifactID := insertMyHomeArtifactFixture(t, repo, "myhome://rsdtRcritNtcList/19833#detail")
	count := 5700

	if _, err := repo.UpsertMyHomeOffering(ctx, noticeID, artifactID, "LH", normalize.OfferingCandidate{
		ApplicationUnitLabel: "전세임대 아파트 5700호",
		SupplyCategory:       "전세임대",
		UnitType:             "아파트",
		SupplyCount:          &count,
		DepositText:          "임대보증금 : 지원한도액 범위 내 전세보증금의 5%",
		MonthlyRentText:      "월임대료 : LH 지원금의 연 1.2~2.2%이자",
		SourceSpan:           "myhome://rsdtRcritNtcList/19833",
		RawRow: map[string]any{
			"source": "myhome_detail",
		},
		Confidence: 0.95,
	}); err != nil {
		t.Fatalf("UpsertMyHomeOffering() error = %v", err)
	...
	if summary.Approved != 1 || summary.Rejected != 0 || summary.Pending != 0 {
		t.Fatalf("summary = %+v, want text-based myhome offering approved", summary)
	}
}

func TestRepository_PromoteOfferingsQA_MyHome0원금액만으로는승인하지않는다(t *testing.T) {
	repo := openCleanTestRepository(t)
	ctx := context.Background()
	noticeID := insertSourceNoticeFixture(t, repo, "20295", "장애인 자립특화형 주택 여기가 입주자 모집공고")
	artifactID := insertMyHomeArtifactFixture(t, repo, "myhome://rsdtRcritNtcList/20295")
	count := 11
	zero := int64(0)

	if _, err := repo.UpsertMyHomeOffering(ctx, noticeID, artifactID, "LH", normalize.OfferingCandidate{
		ApplicationUnitLabel: "매입임대 다가구주택 11호",
		SupplyCategory:       "매입임대",
		UnitType:             "다가구주택",
		SupplyCount:          &count,
		DepositKRW:           &zero,
		MonthlyRentKRW:       &zero,
		SourceSpan:           "myhome://rsdtRcritNtcList/20295",
		RawRow: map[string]any{
			"source": "myhome_api",
		},
		Confidence: 0.95,
	...
	if summary.Approved != 0 || summary.Rejected != 1 || summary.Pending != 0 {
		t.Fatalf("summary = %+v, want zero-money myhome offering rejected", summary)
	}
}
