...
func TestInferSchedulesFromTextArtifacts_PDF깨진범위구분자는구조화한다(t *testing.T) {
	artifacts := []extraction.ExtractedArtifact{
		{
			Type:       extraction.ArtifactTypePDFText,
			SourceSpan: "object://hic-originals/sh/303584/3-notice.pdf",
			RawText:    "■ 청약접수 신청기간 : 2026. 5. 18.(월) 10:00 � 2026. 5. 22.(금) 17:00 신청방법 : SH청약페이지",
		},
	}
	...
	got := schedules[0]
	if !got.StartsAt.Equal(time.Date(2026, 5, 18, 10, 0, 0, 0, time.Local)) {
		t.Fatalf("StartsAt = %s", got.StartsAt)
	}
	if !got.EndsAt.Equal(time.Date(2026, 5, 22, 17, 0, 0, 0, time.Local)) {
		t.Fatalf("EndsAt = %s", got.EndsAt)
	}
}

func TestInferSchedulesFromTextArtifacts_PDF숫자가깨진일정은추측하지않는다(t *testing.T) {
	artifacts := []extraction.ExtractedArtifact{
		{
			Type:       extraction.ArtifactTypePDFText,
			SourceSpan: "object://hic-originals/sh/303584/3-notice.pdf",
			RawText:    "■ 청약접수 신청기간 : 2026. 5. 1�.(월) 10:00 � 2026. 5. 22.(금) 17:00 신청방법 : SH청약페이지",
		},
	}
	...
	if len(schedules) != 0 {
		t.Fatalf("schedules = %+v, want none because the start day is corrupted", schedules)
	}
}
