...
func TestOfferingFromMyHomeItem_상세임대조건문구를금액텍스트로보존한다(t *testing.T) {
	count := 5700
	item := lhdiscovery.MyHomeNoticeItem{
		NoticeID:                 "19833",
		Agency:                   "LH",
		...
		Province:                 "대구광역시",
		SupplyCount:              &count,
		SupportLimitText:         "수도권 14,500만원, 광역시(세종시포함) 11,000만원, 기타 도지역 9,500만원",
		DepositConditionText:     "임대보증금 : 지원한도액 범위 내 전세보증금의 5%",
		MonthlyRentConditionText: "월임대료 : LH 지원금의 연 1.2~2.2%이자",
	}

	offering := OfferingFromMyHomeItem(item, "myhome://rsdtRcritNtcList/19833")

	if offering.DepositText != item.DepositConditionText {
		t.Fatalf("DepositText = %q", offering.DepositText)
	}
	if offering.MonthlyRentText != item.MonthlyRentConditionText {
		t.Fatalf("MonthlyRentText = %q", offering.MonthlyRentText)
	}
	if offering.RawRow["support_limit_text"] != item.SupportLimitText {
		t.Fatalf("RawRow = %+v", offering.RawRow)
	}
}
