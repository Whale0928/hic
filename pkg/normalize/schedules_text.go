...
var applicationSchedulePattern = regexp.MustCompile(`(?s)(청약신청\s*접수기간|신청\s*기간|신청\s*접수|신청접수|접수기간)\s*[:：]?\s*([0-9]{4})\s*[.\-/년]\s*([0-9]{1,2})\s*[.\-/월]\s*([0-9]{1,2})\s*[.\-/일]?\s*(?:\([^)]*\))?\s*(?:(\d{1,2})\s*:\s*(\d{2}))?\s*(?:부터)?\s*[~∼\-�]\s*([0-9]{4})\s*[.\-/년]\s*([0-9]{1,2})\s*[.\-/월]\s*([0-9]{1,2})\s*[.\-/일]?\s*(?:\([^)]*\))?\s*(?:(\d{1,2})\s*:\s*(\d{2}))?`)
