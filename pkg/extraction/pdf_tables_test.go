...
func TestExtractPDFTableRowsFromText_충신동연극인두레주택은성별호점신청단위로추출한다(t *testing.T) {
	rows := ExtractPDFTableRowsFromText(`
2026년 충신동 연극인 두레주택 잔여세대 입주자 모집공고
공급대상 : 총 5호
공급대상 ( 호점 ) 위치 금회 공급 호 전용 면적 ( ㎡ ) 공급여부 공급대상
4 호점 종로구 충신동 1-136 3 호 101 10.54 공급 남성 102 10.72 공급 104 12.55 공급
5 호점 종로구 충신동 1-139 2 호 101 7.31 공급 여성 103 8.17 공급
동일호점 내에서는 동일성별에게만 공급합니다.
남성은 자동으로 4호점, 여성은 자동으로 5호점으로 접수됨 (호점 내 호실은 선택 불가)
임대보증금 및 월임대료
공급대상 ( 호점 ) 위치 호 전용면적 ( ㎡ ) 임대보증금 ( 원 ) 월임대료 ( 원 )
4 호점 종로구 충신동 1-136 101 10.54 1,090,000 149,400 102 10.72 1,090,000 147,200 104 12.55 1,090,000 162,500
5 호점 종로구 충신동 1-139 101 7.31 1,090,000 87,200 103 8.17 1,090,000 118,800
`, "object://hic-originals/sh/304555/1-notice.pdf")

	if len(rows) != 2 {
		t.Fatalf("ExtractPDFTableRowsFromText() len = %d, want 2", len(rows))
	}
	assertPDFTableCell(t, rows[0], "source", "pdf_table_chungshin_theater_dure_gender_supply")
	assertPDFTableCell(t, rows[0], "housing_name", "충신동 연극인 두레주택")
	assertPDFTableCell(t, rows[0], "application_unit_label", "충신동 연극인 두레주택 4호점 남성")
	assertPDFTableCell(t, rows[0], "address", "서울특별시 종로구 충신동 1-136")
	assertPDFTableCell(t, rows[0], "gender_requirement", "남성")
	assertPDFTableCell(t, rows[0], "supply_count", "3")
	assertPDFTableCell(t, rows[0], "deposit_text", "1,090,000원")
	assertPDFTableCell(t, rows[0], "monthly_rent_text", "147,200~162,500원")
	assertPDFTableCell(t, rows[1], "application_unit_label", "충신동 연극인 두레주택 5호점 여성")
	assertPDFTableCell(t, rows[1], "address", "서울특별시 종로구 충신동 1-139")
	assertPDFTableCell(t, rows[1], "gender_requirement", "여성")
	assertPDFTableCell(t, rows[1], "supply_count", "2")
	assertPDFTableCell(t, rows[1], "monthly_rent_text", "87,200~118,800원")
}

func TestExtractPDFTableRowsFromText_마곡도전숙공급유형을신청단위로추출한다(t *testing.T) {
	rows := ExtractPDFTableRowsFromText(`
마곡 도전숙 입주자 모집 공고
공급현황: 총 201호
공급유형별 면적 및 임대료
공급유형 주택유형 공급호수 임대료 세대당 계약면적(㎡) 보증금 (천원) 월 임대료 (원) 전용 공용 기타공용 합계
21㎡형 A형 40 78,480 301,000 21.42 17.52 28.84 67.78 A1형 6 21.46 17.82 28.89 68.17 B형 113 21.19 17.22 28.54 66.95
22㎡형 B1형 9 82,800 318,000 22.60 18.25 30.44 71.29
23㎡형 B2형 12 84,960 325,000 23.25 19.27 31.31 73.83
24㎡형 C형 (주거약자용) 21 89,280 342,000 24.35 19.41 32.79 76.55
`, "object://hic-originals/sh/303858/7-notice.hwp")

	if len(rows) != 4 {
		t.Fatalf("ExtractPDFTableRowsFromText() len = %d, want 4", len(rows))
	}
	assertPDFTableCell(t, rows[0], "source", "hwp_text_magok_challenge_house_supply")
	assertPDFTableCell(t, rows[0], "housing_name", "마곡 도전숙")
	assertPDFTableCell(t, rows[0], "application_unit_label", "마곡 도전숙 21㎡형")
	assertPDFTableCell(t, rows[0], "supply_count", "159")
	assertPDFTableCell(t, rows[0], "deposit_text", "78,480,000원")
	assertPDFTableCell(t, rows[0], "monthly_rent_text", "301,000원")
	assertPDFTableCell(t, rows[0], "exclusive_area_m2", "21")
	assertPDFTableCell(t, rows[3], "application_unit_label", "마곡 도전숙 24㎡형 주거약자")
	assertPDFTableCell(t, rows[3], "application_category", "주거약자")
	assertPDFTableCell(t, rows[3], "supply_count", "21")
	assertPDFTableCell(t, rows[3], "deposit_text", "89,280,000원")
}
